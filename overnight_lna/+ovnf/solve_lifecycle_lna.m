...
%   Reparametrized state (see ovnf.bellman_step_lna):
...
choose_tau = isfield(p,"choose_tau_S") && p.choose_tau_S;
if choose_tau, tau_pol = zeros(N1,N2,N3,T); end
[V(:,:,:,T), c_pol(:,:,:,T), pi_pol(:,:,:,T), tauT] = ...
    ovnf.bellman_step_lna(T, [], p, profile, shocks, ann_price);
if choose_tau, tau_pol(:,:,:,T) = tauT; end
...
for t = T-1 : -1 : 1
    t_step = tic;
    [V(:,:,:,t), c_pol(:,:,:,t), pi_pol(:,:,:,t), taut] = ...
        ovnf.bellman_step_lna(t, V(:,:,:,t+1), p, profile, shocks, ann_price);
    if choose_tau, tau_pol(:,:,:,t) = taut; end
    period_sec(t) = toc(t_step);
